#!/usr/bin/env python3
"""Part-gated shared-prototype training on Stanford Dogs with DINOv2 ViT-B/14.

Geometry is kept exactly aligned with the generated part-box cache:
  original dog image -> official Stanford Dogs union dog bbox crop -> Resize(224,224)
  -> DINOv2 patch tokens.

The HF GroundingDINO cache supplies weak semantic targets for:
  head, ear, muzzle, body, leg, tail.
It is used only for training-time semantic routing / prototype-memory supervision;
inference uses the learned part queries and prototypes only.
"""
...
class CFG:
    ...
    save_dir: str = "./runs/stanforddogs_shared_6parts_k50_dinov2_vitb14"
...
def boxes_to_soft_targets(
    ...
    """Convert [B,P,M,4] weak boxes into normalized local patch targets."""
    if boxes.ndim == 3:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--gdino-train-file", default=cfg.gdino_train_file)
...
def main() -> None:
    ...
    rank0_print("[Run] Stanford Dogs part-gated shared prototype model")
    rank0_print(
        f"[Run] DINO={cfg.dino_model}; classes={cfg.num_classes}; parts={list(cfg.parts)}; "
        f"K={cfg.k_per_part}; geometry=official dog bbox crop -> Resize({cfg.image_size})"
    )
    rank0_print(
    ...
    rank0_print(f"[Data] train={len(train_set)} test={len(test_set)}")
