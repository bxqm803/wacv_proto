...
def main() -> None:
    ...
    if distributed():
        # `pooler_output` uses CLIP's post_layernorm, whereas patch_mean /
        # patch_max read local tokens directly from last_hidden_state. In the
        # latter case post_layernorm has no gradient path, so DDP must permit
        # these two intentionally unused parameters.
        model = DDP(
            model,
            device_ids=[LOCAL_RANK],
            output_device=LOCAL_RANK,
            broadcast_buffers=False,
            find_unused_parameters=(cfg.feature != "cls"),
        )
