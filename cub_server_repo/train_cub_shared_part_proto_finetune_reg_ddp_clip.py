#!/usr/bin/env python3
# NOTE: This file accepts dinov2_vitb14, original dino_vitb16, and clip_vitb16.
...
def load_visual_backbone(model_name: str) -> nn.Module:
    """Load DINOv2, original DINO, or CLIP ViT-B/16 visual backbones."""
    name = str(model_name).lower()
    if is_clip_backbone(name):
        try:
            from transformers import CLIPVisionModel
        except ImportError as e:
            raise ImportError("clip_vitb16 requires transformers.") from e
        model = CLIPVisionModel.from_pretrained("openai/clip-vit-base-patch16")
        model._proto_is_clip = True
        model._proto_is_dino_v1 = False
        return model

    if name in DINO_V1_HUB_MODELS:
...
def extract_patch_tokens(backbone: nn.Module, images: torch.Tensor) -> Tuple[torch.Tensor, int, int]:
    """Return local patch tokens [B,N,C]."""
    if bool(getattr(backbone, "_proto_is_clip", False)):
        ...
        if not isinstance(x, torch.Tensor) or x.ndim != 3:
            raise RuntimeError("CLIP vision tower did not return a token sequence.")
        patch_size = int(getattr(backbone.config, "patch_size", 16))
        expected_n = (images.shape[-2] // patch_size) * (images.shape[-1] // patch_size)
        ...
        if not isinstance(layers, (list, tuple)) or not layers:
            raise RuntimeError("Could not extract original-DINO local tokens.")
        x = layers[-1]
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser("Shared part-prototype DINOv2 / DINO / CLIP ViT finetuning on CUB")
    ...
    p.add_argument("--dino-model", default=cfg.dino_model, help="dinov2_vitb14, dino_vitb16, or clip_vitb16")
...
def main() -> None:
    ...
    try:
        ...
        norm_name = "CLIP" if is_clip_backbone(cfg.dino_model) else "ImageNet"
        rank0_print(f"[Input] normalization={norm_name}; geometry=bird-crop -> Resize({cfg.image_size})")
