...
def atomic_torch_save(obj, path: Path) -> None:
    """Write a valid checkpoint atomically so an interrupted save never corrupts resume state."""
    tmp_path = path.with_name(path.name + ".tmp")
    torch.save(obj, tmp_path)
    os.replace(tmp_path, path)


def empty_results(split_name, args):
    return {
        "split": split_name,
...
def validate_resume_results(results, split_name, args, out_pt: Path) -> None:
    required = [
        "img_ids", "relpaths", "visible_parts", "selected_boxes_xyxy",
        "selected_scores", "selected_areas", "raw_candidate_counts",
    ]
    missing = [k for k in required if k not in results]
    if missing:
        raise KeyError(f"Cannot resume {out_pt}: missing keys {missing}")

    lengths = {k: len(results[k]) for k in required}
    if len(set(lengths.values())) != 1:
        raise ValueError(f"Cannot resume {out_pt}: inconsistent list lengths {lengths}")

    if results.get("split") != split_name:
        raise ValueError(
            f"Cannot resume {out_pt}: split={results.get('split')} but requested {split_name}. "
            "Use a different output directory or --no-resume."
        )

    for key, expected in [("image_size", args.image_size), ("box_thr", args.box_thr), ("text_thr", args.text_thr)]:
        got = results.get(key)
        if got is not None and got != expected:
            raise ValueError(
                f"Cannot resume {out_pt}: cached {key}={got}, requested {expected}. "
                "Use the same settings or --no-resume."
            )


def restore_part_counters(results):
    visible_parts = results.get("visible_parts", [])
    run_parts = sum(sum(bool(v) for v in row) for row in visible_parts)
    skip_parts = len(visible_parts) * len(PARTS) - run_parts
    return run_parts, skip_parts
...
def main():
    ...
    parser.add_argument(
        ...
        help="Atomically save the split cache every N newly processed images; 0 saves only at the end.",
    )
    parser.add_argument(
        "--no-resume", action="store_true",
        help="Ignore an existing split cache and rebuild it from scratch.",
    )
    ...
    for split_name, split_flag in split_specs:
        ...
        if out_pt.exists() and not args.no_resume:
            results = torch.load(out_pt, map_location="cpu")
            validate_resume_results(results, split_name, args, out_pt)
            done_img_ids = {int(x) for x in results["img_ids"]}
            num_run_part, num_skip_part = restore_part_counters(results)
            print(
                f"[{split_name}] resume: loaded {len(done_img_ids)}/{len(img_ids)} images from {out_pt} "
                f"(run_parts={num_run_part}, skip_parts={num_skip_part})"
            )
        else:
            results = empty_results(split_name, args)
            done_img_ids = set()
            num_run_part = 0
            num_skip_part = 0
        ...
        for idx, img_id in enumerate(img_ids, 1):
            ...
            if args.save_every > 0 and newly_processed % args.save_every == 0:
                atomic_torch_save(results, out_pt)
                print(
                    f"[{split_name}] checkpoint saved: {len(results['img_ids'])}/{len(img_ids)} images -> {out_pt}"
                )

            if idx % args.log_every == 0 or idx == len(img_ids):
                print(
                    f"[{split_name}] {idx}/{len(img_ids)} "
                    f"cached={len(results['img_ids'])} run_parts={num_run_part} skip_parts={num_skip_part}"
                )
